...
func TestProxy(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			c := &MockClient{
				RunFunc: func(ctx context.Context, got *protocol.Request) (*protocol.Response, error) {
					if !reflect.DeepEqual(test.serverReq, got) {
...
func TestProxyLive(t *testing.T) {
	addr := flag.String("addr", "", "broker addr")
	flag.Parse()
	if *addr == "" {
		t.Skip()
	}
	...
	conn, err := jocko.Dial("tcp", *addr)
	if err != nil {
		t.Fatal(err)
	}
	c := &kafkaproxy.Conn{conn}
...
func timeoutmw(timeout time.Duration) kafkaproxy.Middleware {
	return func(next kafkaproxy.Endpoint) kafkaproxy.Endpoint {
		return func(ctx context.Context, request *protocol.Request) (*protocol.Response, error) {
			req := request.Body.(*protocol.ProduceRequest)
			req.Timeout = timeout
			response, err := next(ctx, request)
			res := response.Body.(*protocol.ProduceResponse)
